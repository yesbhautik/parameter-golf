"""Domination V3: compact no-TTT path for 10-min track."""
...
def eval_val_sliding(args, base_model, rank, world_size, device, val_tokens,
    ...
    for bi in range(0, len(my_windows), batch_seqs):
        ...
        for i, ws in enumerate(bw):
            ep = min(ws + seq_len, total_tokens)
            wl = ep - ws
            wlens.append(wl)
        ...
        if rank == 0 and (bi // batch_seqs) % 50 == 0:
            ...
            if tc.item() > 0:
                rl = (ls / tc).item()
                rb = rl / math.log(2.0) * (tc.item() / bc.item())
...
FP16_PASSTHROUGH_PATTERNS = tuple(
    p.strip() for p in os.environ.get("FP16_PASSTHROUGH_PATTERNS", "").split(",") if p.strip()
)
...
def mixed_quantize(state_dict, int6_cats):
    ...
    for name, tensor in state_dict.items():
        ...
        if t.is_floating_point() and FP16_PASSTHROUGH_PATTERNS and any(p in name for p in FP16_PASSTHROUGH_PATTERNS):
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids, target_ids):
        ...
        x = self.final_norm(self._run_blocks(x0, x0))
        x_flat = x.reshape(-1, x.size(-1))
        return F.cross_entropy(self._logits(x_flat).float(), target_ids.reshape(-1), reduction="mean")
...
def main():
    ...
    deq_sd = dequantize_mixed(qs["w"], qs["m"], sd_cpu)
    ...
    if args.eval_stride > 0 and args.eval_stride < args.train_seq_len:
        ...
        qvl, qvb = eval_val_sliding(args, eval_model, rank, world_size, device, val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut, stride=args.eval_stride, batch_seqs=args.eval_batch_seqs)
    else:
        qvl, qvb = eval_val(args, eval_model, rank, world_size, device, grad_accum_steps, val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut)
