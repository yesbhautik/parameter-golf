"""
Unbeatable v5: 11-technique stack. Combines PR #162's novel modules (SmearGate,
BigramHash, OrthoInit, Muon WD, SWA) with our STE fake-int6 QAT + fp16 embedding
passthrough + zstd-22 + sliding window eval + tuned Muon.

Supports both val-only and standard training via DATA_PATH.
SWA controlled by SWA_ENABLED env var (disable for val-only memorization).
"""
...
class Hyperparameters:
    ...
    weight_decay = float(os.environ.get("WEIGHT_DECAY", 0.034))
    muon_wd = float(os.environ.get("MUON_WD", 0.034))
    ...
    with torch.inference_mode():
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            for i, ws in enumerate(bw):
                ep = min(ws + seq_len, total_tokens); wl = ep - ws; wlens.append(wl)
                ch = val_tokens[ws:ep + 1].to(dtype=torch.int64, device=device)
            ...
            for i, ws in enumerate(bw):
                wl = wlens[i]; s = 0 if ws == 0 else wl - stride
...
FP16_PASSTHROUGH_PATTERNS = tuple(p for p in os.environ.get("FP16_PASSTHROUGH_PATTERNS", "tok_emb,blocks.10.attn.c_k").split(",") if p)
...
PACK_INTQ_ENABLED = bool(int(os.environ.get("PACK_INTQ_ENABLED", "0")))
...
def _pack_uint_codes(codes_u8, bits):
    if bits not in (5, 6):
        raise ValueError(f"Unsupported packed bits: {bits}")
    flat = codes_u8.reshape(-1).astype(np.uint8, copy=False)
    bit_lanes = ((flat[:, None] >> np.arange(bits, dtype=np.uint8)) & 1).astype(np.uint8)
    packed = np.packbits(bit_lanes.reshape(-1), bitorder="little")
    return packed

def _unpack_uint_codes(packed_u8, bits, count):
    if bits not in (5, 6):
        raise ValueError(f"Unsupported packed bits: {bits}")
    bits_flat = np.unpackbits(packed_u8.astype(np.uint8, copy=False), bitorder="little")
    bits_flat = bits_flat[: count * bits]
    lanes = bits_flat.reshape(count, bits).astype(np.uint16, copy=False)
    weights = (1 << np.arange(bits, dtype=np.uint16))
    return (lanes * weights).sum(axis=1).astype(np.uint8, copy=False)
...
def quantize_state_dict_int6(state_dict):
    ...
    for name, tensor in state_dict.items():
        ...
        if any(pat in name for pat in FP16_PASSTHROUGH_PATTERNS):
            passthrough_orig_dtypes[name] = str(t.dtype).removeprefix("torch.")
            kept = t.to(dtype=torch.float16).contiguous(); passthrough[name] = kept; stats["int8_payload_bytes"] += tensor_nbytes(kept); continue
        if t.numel() <= INT8_KEEP_FLOAT_MAX_NUMEL:
            if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS): kept = t.float().contiguous()
            elif t.dtype in {torch.float32, torch.bfloat16}:
                passthrough_orig_dtypes[name] = str(t.dtype).removeprefix("torch."); kept = t.to(dtype=INT8_KEEP_FLOAT_STORE_DTYPE).contiguous()
            else: kept = t
            passthrough[name] = kept; stats["int8_payload_bytes"] += tensor_nbytes(kept); continue
        ...
        if PACK_INTQ_ENABLED and qtype in {"int6", "int5"}:
            bits = 6 if qtype == "int6" else 5
            qmax = 31 if qtype == "int6" else 15
            codes = (q.to(torch.int16) + qmax).clamp_(0, (1 << bits) - 1).to(torch.uint8).cpu().numpy()
            packed = _pack_uint_codes(codes, bits=bits)
            q = torch.from_numpy(packed).contiguous()
            qmeta_entry = {"scheme": "per_row", "axis": 0, "qtype": qtype, "packed_bits": bits, "shape": shape, "zero_point": qmax}
            if s.ndim == 0:
                qmeta_entry["scheme"] = "scalar"
                qmeta_entry.pop("axis", None)
    ...
    quant_format = "mixed_int6_int8_packed_fp16embed_v3" if PACK_INTQ_ENABLED else "mixed_int6_int8_fp16embed_v2"
    obj = {"__quant_format__": quant_format, "quantized": quantized, "scales": scales, "dtypes": dtypes, "passthrough": passthrough}
...
def dequantize_state_dict(obj):
    out, qmeta, pod = {}, obj.get("qmeta", {}), obj.get("passthrough_orig_dtypes", {})
    for name, q in obj["quantized"].items():
        dtype = getattr(torch, obj["dtypes"][name]); s = obj["scales"][name]
        meta = qmeta.get(name, {})
        if "packed_bits" in meta:
            bits = int(meta["packed_bits"]); shape = tuple(int(d) for d in meta["shape"]); count = int(np.prod(shape))
            zp = int(meta.get("zero_point", 31))
            codes = _unpack_uint_codes(q.detach().to("cpu").contiguous().numpy(), bits=bits, count=count)
            q_signed = torch.from_numpy(codes.astype(np.int16, copy=False)).view(*shape).to(torch.float32).sub_(float(zp))
            if s.ndim > 0:
                s = s.to(dtype=torch.float32)
                out[name] = (q_signed * s.view(shape[0], *([1] * (len(shape) - 1)))).to(dtype=dtype).contiguous()
            else:
                out[name] = (q_signed * float(s.item())).to(dtype=dtype).contiguous()
...
def main():
    ...
    base_model.load_state_dict(dequantize_state_dict(torch.load(io.BytesIO(rd), map_location="cpu")), strict=True)
