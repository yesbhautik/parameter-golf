"""
Combined optimal submission: val-only training + sliding window eval + 10 layers
with mixed int8/int6 quantization + tuned Muon optimizer hyperparameters.

Techniques sourced from:
- PR #44: val-only training (organizer-approved via Discord)
- PR #50: sliding window evaluation (stride=64, every token scored with 960+ context)
- PR #39: 10 layers + mixed int6/int8 quantization (extra capacity under 16MB)
- PR #52: tuned Muon (momentum 0.99, lower LR, longer warmdown, seq_len 4096)
"""
...
# HYPERPARAMETERS
# -----------------------------
# Combined optimal: 10 transformer blocks at width 512, seq_len 4096,
# tuned Muon (momentum 0.99, lower LR), sliding window eval (stride 64),
# mixed int8/int6 quantization for middle layers.
...
class Hyperparameters:
    ...
    # Sliding window eval: stride < train_seq_len enables overlapping windows
    eval_stride = int(os.environ.get("EVAL_STRIDE", 64))
    eval_batch_seqs = int(os.environ.get("EVAL_BATCH_SEQS", 64))

    # Mixed-precision quantization: comma-separated layer indices for int6
    int4_layers = os.environ.get("INT4_LAYERS", "3,4,5,6,7")
...
class GPT(nn.Module):
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        Used by sliding window evaluation."""
        x = self.tok_emb(input_ids)
        x = F.rms_norm(x, (x.size(-1),))
        x0 = x
        x = self._run_blocks(x, x0)
...
def main() -> None:
    ...
    log0(f"int4_layers:{args.int4_layers} int4_step:{args.int4_step}")
    ...
    # Mixed-precision: round middle layers to int6 (64 levels) for better zlib compression.
    # This lets a 10-layer model fit under the 16MB cap.
    if args.int4_layers:
    ...
    # Use sliding window eval for the final post-quant roundtrip validation
